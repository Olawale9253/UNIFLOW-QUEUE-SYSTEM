...
function StaffAppointments() {
    ...
    useEffect(() => {
        api.get('/offices/active').then((response) => {
            const activeOffices = response.data || [];
            setOffices(activeOffices);
            if (activeOffices[0]) setOfficeId(activeOffices[0].id);
    ...
    return <StaffLayout>
        <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
            ...
            <div className="flex items-center gap-3"><label className="text-sm font-medium text-gray-700 dark:text-gray-300">Office</label><select value={officeId} onChange={(event) => setOfficeId(event.target.value)} className="rounded-lg border px-3 py-2 dark:border-gray-600 dark:bg-gray-700 dark:text-white">{offices.map((office) => <option key={office.id} value={office.id}>{office.name}</option>)}</select><button onClick={() => loadAppointments()} className="rounded-lg bg-blue-600 px-4 py-2 text-white">Refresh</button></div>
        </div>
