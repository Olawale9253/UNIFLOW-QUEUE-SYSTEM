...
function StaffAppointments() {
    ...
    const visibleAppointments = appointments
        .filter((appointment) => {
            ...
            return appointmentTime >= now && !['COMPLETED', 'CANCELLED'].includes(appointment.status);
        })
    ...
    return <StaffLayout>
        ...
        <div className="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
            {loading ? <p className="p-8 text-center text-gray-500">Loading appointments...</p> : visibleAppointments.length === 0 ? <p className="p-8 text-center text-gray-500">No upcoming appointments for this office.</p> : <div className="divide-y dark:divide-gray-700">{visibleAppointments.map((appointment) => <div key={appointment.id} className="flex flex-wrap items-center justify-between gap-4 p-5"><div><p className="font-semibold text-gray-900 dark:text-white">{new Date(appointment.appointmentTime).toLocaleDateString([], { weekday: 'short', month: 'short', day: 'numeric' })} · {new Date(appointment.appointmentTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p><p className="text-sm font-medium text-gray-700 dark:text-gray-300">{appointment.studentName || 'Student'} · {appointment.serviceName}</p><p className="text-sm text-gray-500 dark:text-gray-400">{appointment.referenceNumber} · {appointment.status}</p></div><div className="flex gap-2">{appointment.status === 'PENDING' && <button onClick={() => updateAppointment(appointment.id, 'confirm')} className="rounded-lg bg-green-600 px-3 py-2 text-sm text-white">Confirm</button>}{!['COMPLETED', 'CANCELLED'].includes(appointment.status) && <button onClick={() => updateAppointment(appointment.id, 'complete')} className="rounded-lg bg-blue-600 px-3 py-2 text-sm text-white">Complete</button>}{!['COMPLETED', 'CANCELLED'].includes(appointment.status) && <button onClick={() => updateAppointment(appointment.id, 'staff-cancel', 'delete')} className="rounded-lg bg-red-600 px-3 py-2 text-sm text-white">Cancel</button>}</div></div>)}</div>}
        </div>
    </StaffLayout>;
}
